...
現在の到達点は `docs/uft-v14.0.md`(第十四期統合。Rust シミュレーション 59 本 314 PASS / 0 FAIL + Lean 4 定理 6 本、全スイートは results/v140_full_suite.txt — T⁴ 系 4 本が重く全スイートは ~3 時間)。第十四期で**対の物語が完結**: Lanczos 疎ソルバを自作し (v13.1、N=6 T⁴ を 6,700 倍速)、N=18 の傾き T⁴ を再走査して「格子上の指数は (磁束,N) の数論」「素数 3 が 2 タワーの抑制の掛け算を構造的に禁じる」を確立 (v13.2) — 「σ は因子化近似の影」仮説は棄却され、**σ はコンパクト化の既約な離散データ** (v10.3) が対抗仮説の棄却で確立した。**続きの作業は v14.0 §3 の残高から選ぶ**(筆頭: geometric-yukawa 論文の本文化(対の物語 5b/5c が核)、anomaly-search 論文の本文化、v4.3 域 Lean の配列ベース列挙、格子指数の数論の定式化)。
