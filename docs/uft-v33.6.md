...
- **n = 3 の全 7 セルで dense と裁定が完全一致**: site [2,2,2]・entangler [2,4]・
  ...
  radical が正しく数える。
- **48 qubit の証明書 (行列なし)**: site 96 本 → Exact [2×48]・
  +entangler → [2×46, 4]・Z₄₈ 欠落 → SuperselectionSectors [(2⁴⁷, 1)×2] — 全て
  96×96 の GF(2) rank で決まり、**2^48 次元の行列はどこにも現れない**。
  *(器械訂正 [v34.0-B]: 初版は壁時計バーを合否条件に含んでいたが、JOBS=12 の
  儀式で並列負荷により超過し FAIL — 壁時計は「並列化で結果が変わらない」規約に
  反する器械バグとして計時を合否から除外した。裁定・数値は不変。)*
