//! v23.2 δ⟨K_A⟩ の距離分解 — R₂ 不足の予算を測る (第二十四期)
//!
//! v23.1 の確定事項: K_A の x-NN プロファイルは線形 BW と 1% 一致 (放物仮説は反証)。
//! よって R₂ 不足は K_A の**非近接構造**が担う。本版は真の modular 応答
//! δ⟨K_A⟩ = Σ_{ab} K[a,b]·δC[b,a] をペア種別に分解する:
//!   対角 (a=b) / x-NN / y-NN / z-NN / x-NNN (Δx=2) / 横 NNN / それ以遠 (長距離)。
//! K_boost が持つのは x-NN + 横-NN のみ — 取り逃し予算が直接読める。
//! δC は rank-2 更新式 (α = 0.005 中心差分)、K_A は DD で厳密構成。
//!
//! 装置ゲート: [G1/G2] v22.7 の DD 経路, [G4] バケット総和 = δ⟨K_A⟩ (恒等式 1e-8)。
//! 事前登録: (a) 非近接 (NNN 以遠) の寄与が有効 d のどこかで ≥ 10% = 「非近接構造が
//!   R₂ 不足の担い手」の定量確認 → 次版で K_boost 改良形 (NN + 実測非近接) を検定 /
//!   (a′) 非近接 < 10% 一様 = 不足は横-NN の重み配分 (再パズル化 — それも決定情報) /
//!   (b) ゲート故障。d ∈ {0.5, 1.5, 2.5}。
...
fn main() {
    self_test();
    println!("=== v23.2 δ⟨K_A⟩ の距離分解 — R₂ 不足の予算 (第二十四期) ===\n");
    println!("事前登録: (a) 非近接 ≥ 10% = 担い手の定量化 / (a′) < 10% = NN 配分に再帰着\n");
    ...
    for &(_, fr) in &budget_rows {
        let far = (fr[4] + fr[5] + fr[6]).abs();
        max_far = max_far.max(far);
    }
    let branch_a = nfail == 0 && max_far >= 0.10;
    println!(
        "\n[判定] {}",
        if nfail > 0 {
            "装置ゲート故障 — 記録"
        } else if branch_a {
            "事前登録 (a): 非近接 (NNN 以遠) が δ⟨K_A⟩ の ≥10% を担う — R₂ 不足の担い手を定量化。次版で K_boost 改良形を検定"
        } else {
            "事前登録 (a′): 非近接 < 10% 一様 — 不足は NN の重み配分に再帰着 (記録)"
        }
    );
    println!("    max 非近接寄与 = {:.1}%", max_far * 100.0);

    let j = Json::Obj(vec![
        ("version".into(), Json::Str("v23.2".into())),
        (
            "rows".into(),
            Json::Arr(
                budget_rows
                    ...
                    .map(|&(d, fr)| {
                        Json::Obj(vec![
                            ("d".into(), Json::Num(d)),
                            ("diag".into(), Json::Num(fr[0])),
                            ("xnn".into(), Json::Num(fr[1])),
                            ("ynn".into(), Json::Num(fr[2])),
                            ("znn".into(), Json::Num(fr[3])),
                            ("xnnn".into(), Json::Num(fr[4])),
                            ("tnnn".into(), Json::Num(fr[5])),
                            ("far".into(), Json::Num(fr[6])),
                        ])
                    })
                    .collect(),
            ),
    ...
    let p = write_artifact("results/v232_kbudget.json", &j.render());
    println!("\n[artifact] {}", p);
    println!(
        "\n総合判定: {}",
        if nfail == 0 {
