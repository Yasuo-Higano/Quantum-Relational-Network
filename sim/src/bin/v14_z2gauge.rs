...
fn main() {
    println!("=== v1.4 創発ゲージ場: 3D Z2 格子ゲージ理論 (L={}) ===\n", L);
    let beta_c = -0.5 * (0.221654f64.tanh()).ln();
    println!("3D Ising 双対性による厳密な転移点: β_c = -½ ln tanh(0.221654) = {:.4}\n", beta_c);
    println!("  β      ⟨P⟩      χ(2)      χ(3)     [χ(R)=Creutz比→弦張力σ]   強結合予言 -ln tanh β");
    ...
    println!("          β_c={:.3} を境に χ(3) が急落することを確認せよ。", beta_c);
    println!("\n結論: ゲージ構造は基本法則に書き込まなくても、量子ビットの海の相として創発しうる");
    println!("      (Wen: 光子は弦ネット凝縮の集団励起)。さらに双対性 (ゲージ理論 ⇔ Ising 磁石) は、");
    println!("      「同じ実在の異なる記述」が可能なことを示す — 基本変数は一意でない (群盲象の教訓)。");
