...
fn main() {
    ...
    // ---- [B] 世代数 = 磁束のトポロジー ----
    println!("[B] 磁束 Q を貫いた {}×{} トーラス: 最低準位の縮退度 = Q (指数定理)", 12, 12);
    let n = 12usize;
    let ns = n * n;
    for q in 1..=4usize {
        ...
        println!(
            "  Q={}: 最低準位群 {:?}... → 縮退度 {} (ギャップ {:.3})  {}",
            q,
            &evs[0..5.min(ns)].iter().map(|x| format!("{:.4}", x)).collect::<Vec<_>>(),
            best.1,
            best.0,
            pass(best.1 == q)
        );
    }
    println!("  => 縮退度 = 磁束量子数 Q。ディラック場では「カイラルゼロモードの数 = Q」(指数定理)。");
    println!("     隠れ空間に位相的な捻れ Q=3 があれば、同じ場が 3 世代に見える —");
    println!("     ***「なぜ 3 世代?」は「隠れ空間のトポロジカル不変量はいくつか?」に翻訳される***\n");
    ...
    println!("  [SU(3)]²U(1): {:+.2e}  [SU(2)]²U(1): {:+.2e}  [grav]²U(1): {:+.2e}  [U(1)]³: {:+.2e}",
        a3, a2, ag, ay);
    println!("  Witten SU(2) 大域アノマリー: 二重項の数 = {} (偶数なら無矛盾)", ndoublets);
    let ok = a3.abs() < 1e-12 && a2.abs() < 1e-12 && ag.abs() < 1e-12 && ay.abs() < 1e-12
        && (ndoublets as i64) % 2 == 0;
    println!("  => 4 つの三角アノマリー + 大域アノマリーが全て消える  {}", ok_str(ok));
    // 部分集合では消えない
    let ay_q: f64 = gen[..3].iter().map(|f| f.1 * f.2 * f.3.powi(3)).sum();
    let ay_l: f64 = gen[3..].iter().map(|f| f.1 * f.2 * f.3.powi(3)).sum();
    println!("  クォークだけ: [U(1)]³ = {:+.4} ≠ 0 / レプトンだけ: {:+.4} ≠ 0", ay_q, ay_l);
    ...
    println!("  解: Y_Q = {:.4} (SM: 1/6), Y_u^c = {:.4} (SM: -2/3), Y_d^c = {:.4} (SM: 1/3),",
        a_, c, b);
    ...
    let sm_ok = (a_ - 1.0 / 6.0).abs() < 1e-12
        ...
        && (b - 1.0 / 3.0).abs() < 1e-12
        && (d + 0.5).abs() < 1e-12;
    println!("  => アノマリー消去だけで超電荷は標準模型の値に一意に決まる  {}", ok_str(sm_ok));
    // 帰結: 水素原子の中性
    let q_u = 0.5 + a_;
    let q_d = -0.5 + a_;
    let q_e = -0.5 + d;
    println!("  帰結: Q_p + Q_e = (2Q_u+Q_d) + Q_e = {:+.1e} — 陽子と電子の電荷が正確に相殺", 2.0 * q_u + q_d + q_e);
    println!("  ***水素原子が中性である (10^-21 の精度で検証済) のは、アノマリー消去の帰結でありうる***");
