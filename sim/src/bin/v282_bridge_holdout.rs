//! v28.2 relational geometry bridge — holdout 執行 (第二十九期 II/III, 調整禁止)
//!
//! 事前登録 = bridge_candidates.yml (v28.0) + 判定バー = v28.1 の FROZEN 節
//! (コミット 5ac98b5 で凍結)。本バイナリは holdout 4 系を**初めて**走らせ、
//! 凍結バーで候補 B1–B4 を採点する。**バー外の候補は調整せず棄却する** — 棄却は
//! 実験の正しい帰結であり器械の故障ではないので、[FAIL] トークンは器械破れ
//! (凍結節不一致・回帰アンカー逸脱・採点不能) だけに使い、候補の成績は
//! [採点] 行 (生存/棄却) で記録する (suite の FAIL 計数を汚染しない)。
//!
//! [H0] 凍結の機械検査: 本ファイルの FROZEN BRIDGE READOUT 節は v281 から逐語
//! 複製 — 両ファイルの節 SHA-256 の一致を検査し、TRAIN-1 の B1 標本値を再計算
//! して v281 の公表 JSON と照合する (複製コードが同一の数を出す回帰アンカー)。
//!
//! holdout (実行前宣言 = docs/uft-v28.1.md §3):
//!   [H1] 開放端鎖 N=101 (非一様ホッピング窓): Path・端点 2・隣接 100%・橋 0・
//!        MDS λ2/λ1 ≤ 1e-8 (B1/B2/B3)
//!   [H2] Z2GaugeRing l=14 (w=1, h=0.6, m=0.2 — 閉じ込め相, 相互作用系):
//!        Cycle 14・隣接 100%・橋 0 (B1/B2/B3) + B4 (apply_bond_op) の到着順 vs
//!        B1 再構成距離 Spearman ≥ 0.85 + 未使用チャネル (B1 幾何 → B3 予言,
//!        d ≤ 5 で Spearman ≥ 0.90)
//!   [H3] 同一連続理論の二離散化 (staggered N=402 vs Wilson N=402 [2 成分/site],
//!        m_phys·a = 0.05): 両方 Cycle・隣接 100%・ξ_stag/ξ_wil ∈ [0.8, 1.25]。
//!        ξ 推定則 (本ファイルで一回だけ宣言・全系/全核共通): 再構成 hop 距離の
//!        偶数 d ∈ {4,6,…,20} で ⟨ln w⟩ を線形フィット (副格子交代を避ける)
//!   [H4] Lorentzian 整合: リング N=202 — t* vs d_B1 (再構成距離のみ) 線形
//!        R² ≥ 0.98・源間 v ばらつき ≤ 3% / 開放鎖 — 無巻き付き (path 距離線形)
//!
//! 解釈の上限: 全通過でも C3 (toy 機構) — bridge law 登録簿は空のまま。
...
/// 実対称 H (一体) の基底状態相関行列 (最低 nocc 準位を占有)
...
/// HOLD-1: 開放端鎖 N=101, 非一様ホッピング t_x = 1 + 0.15·exp(−((x−50)/12)²)
...
/// HOLD-3a: staggered リング (hopping −1/2, 質量 m·(−1)^x)
...
/// HOLD-3b: 1D Wilson リング (2 成分/site, r = 1)。
/// h(k) = σ₁ sin k + σ₃ (m + 1 − cos k) — C は k 空間射影子 P₋ から解析構成
...
/// ξ 推定 (宣言: 全系・全核共通): 再構成 hop 距離の偶数 d ∈ {4,6,…,20} で
/// ⟨ln w(d)⟩ を距離ごとに平均し線形フィット → ξ = −1/傾き
fn xi_from_order(w: &[f64], n: usize, order: &[usize], cyclic: bool) -> f64 {
...
fn main() {
    ...
    // 候補の成績 (凍結バーの判定 — [FAIL] トークンを使わない)
    let mut score: BTreeMap<&str, Vec<(String, bool)>> = BTreeMap::new();
    let mut judged: Vec<(String, bool, String)> = Vec::new();
    let mut judge = |score: &mut BTreeMap<&str, Vec<(String, bool)>>,
        ...
        judged.push((format!("{} × {}", cand, sys), bar_ok, detail));
    };
    ...
    // ---- [H0] 凍結の機械検査 ----
    {
        ...
        let s282 = read_src("sim/src/bin/v282_bridge_holdout.rs");
        let (c281, c282) = (cut(&s281), cut(&s282));
        let sha_ok = match (&c281, &c282) {
            (Some(a), Some(b)) => sha256_hex(a.as_bytes()) == sha256_hex(b.as_bytes()),
            _ => false,
        };
        // TRAIN-1 B1 標本の回帰アンカー
        let ring = RingChain { n: 202 };
        let g = ring.init();
        let st = NodeState {
            nodes: 202,
            m: 1,
        ...
        let perm = make_perm(202, 20260729);
        let shuf = shuffle_state(&st, &perm);
        let w = w_b1_gauss(&shuf);
        let json = std::fs::read_to_string("results/v281_bridge_train.json")
            .or_else(|_| std::fs::read_to_string("../results/v281_bridge_train.json"))
            .unwrap_or_default();
        let mut anchor_ok = false;
        let mut max_dev = f64::INFINITY;
        if let Some(p) = json.find("\"train1_b1_sample\":") {
            let rest = &json[p..];
            if let (Some(a), Some(b)) = (rest.find('['), rest.find(']')) {
                let vals: Vec<f64> = rest[a + 1..b]
                    .split(',')
                ...
                if vals.len() == 5 {
                    max_dev = vals
                        .iter()
                        .enumerate()
                        .map(|(k, &v)| (w[k + (k + 1) * 202] - v).abs())
                        .fold(0.0, f64::max);
                    anchor_ok = max_dev < 1e-12;
                }
            }
        }
        let sha_str = c281
            .as_ref()
            .map(|a| sha256_hex(a.as_bytes())[..16].to_string())
            .unwrap_or_default();
        check(
            "[H0] 凍結節 SHA-256 一致 (v281 = v282) + TRAIN-1 B1 標本の回帰アンカー",
            sha_ok && anchor_ok,
            format!("sha 一致 {} ({}…) / アンカー最大差 {:.1e}", sha_ok, sha_str, max_dev),
        );
    }
    ...
    let mut h1_b1_order: Vec<usize> = Vec::new();
    for (name, w) in [
        ...
        let mds_ok = if path_ok {
            ...
            for i in 0..n {
                for j in 0..n {
                    let d2 = ((i as f64) - (j as f64)).powi(2);
                    rowmean[i] += d2;
                    total += d2;
                }
            }
            for r_ in rowmean.iter_mut() {
                *r_ /= n as f64;
            }
            total /= (n * n) as f64;
            for i in 0..n {
            ...
            (ev[1] / ev[0]).abs() <= BAR_PATH_MDS
        } else {
            false
        };
        let ok = path_ok && adj >= BAR_ADJACENCY && r.bridges.is_empty() && mds_ok;
        judge(
            &mut score,
            match name {
                "B1" => "B1",
        ...
        if name == "B1" && path_ok {
            h1_b1_order = r.comps[0].order.clone();
        }
    }

    // ---- [H2] Z2 ゲージ環 l=14 (相互作用系) ----
    {
        let l = 14;
        let ring = Z2GaugeRing::try_new(l, 7, 1.0, 0.6, 0.2, vec![]).expect("Z2 l=14");
        let (e0, gs, res) = ring.ground_state(11);
        check(
            "[H2] Z2 基底状態の器械 (Lanczos 収束)",
            res < 1e-6,
            format!("E₀ = {:.9} (dim {}, 残差 {:.1e})", e0, gs.dim(), res),
        );
        let perm = make_perm(l, 2718);
        let view = Z2View {
            l,
            masks: &gs.masks,
            psi: &gs.psi,
        ...
        let mut b1_order: Vec<usize> = Vec::new();
        for (name, w) in [
            ("B1", w_b1_z2(&view)),
            ("B2", w_b2_z2(&view)),
            ("B3", w_b3_z2(&view)),
            ...
            let cyc =
                r.comps.len() == 1 && r.comps[0].topology == 0 && r.comps[0].order.len() == l;
            ...
            judge(
                ...
                "HOLD-2 Z2 (相互作用)",
                ok,
                format!("Cycle={} 隣接 {:.4} 橋 {}", cyc, adj, r.bridges.len()),
            );
            if name == "B1" && cyc {
                b1_order = r.comps[0].order.clone();
            }
        }
        // 未使用チャネル: B1 幾何が B3 チャネルの順序を予言 (d ≤ 5)
        {
            let ok_dist = !b1_order.is_empty();
            let (sp, npair) = if ok_dist {
                let mut pos = vec![0usize; l];
                for (t, &nd) in b1_order.iter().enumerate() {
                    pos[nd] = t;
                }
                let wb3 = w_b3_z2(&view);
                let mut xs = Vec::new();
                let mut ys = Vec::new();
                for i in 0..l {
                    for j in (i + 1)..l {
            ...
            judge(
                &mut score,
                "B1",
                "HOLD-2 未使用チャネル (B1 幾何 → B3 予言)",
                ok_dist && sp >= BAR_Z2_UNUSED,
                format!("Spearman = {:.4} (バー {:.2}, {} 対)", sp, BAR_Z2_UNUSED, npair),
            );
        }
        // B4: apply_bond_op クエンチ → 密度前線の到着順 vs B1 再構成距離
        {
            let base: Vec<f64> = (0..l).map(|s| gs.density(s)).collect();
            let bond_x = 0usize;
            let mut s = ring.apply_bond_op(&gs, bond_x);
            ...
            for it in 1..=nsteps {
                s = ring.step(&s, EvolutionParameter(dt));
                let t = dt * it as f64;
                for site in 0..l {
            ...
            let ok_dist = !b1_order.is_empty();
            let (sp, n_reached) = if ok_dist {
                let mut pos = vec![0usize; l];
                for (t, &nd) in b1_order.iter().enumerate() {
                    pos[nd] = t;
                }
                let src0 = (0..l).find(|&i| perm[i] == bond_x).unwrap();
                let src1 = (0..l).find(|&i| perm[i] == (bond_x + 1) % l).unwrap();
                let d_by_new: Vec<f64> = (0..l)
                    ...
                    .collect();
                let t_by_new: Vec<f64> = (0..l).map(|i| tstar[perm[i]]).collect();
                (
                    spearman(&d_by_new, &t_by_new),
                    t_by_new.iter().filter(|t| t.is_finite()).count(),
                )
            } else {
                (0.0, 0)
            };
            judge(
                ...
                "B4",
                "HOLD-2 因果 (相互作用系)",
                ok_dist && sp >= BAR_Z2_SPEARMAN,
                format!(
                    "Spearman = {:.4} (バー {:.2}) / 到達 {}/{}",
                    sp, BAR_Z2_SPEARMAN, n_reached, l
                ),
            );
        }
    }

    // ---- [H3] 二離散化 (staggered vs Wilson) ----
    {
        let n = 402;
        let m_phys = 0.05;
        ...
        for (name, ws, ww) in [
            ...
            for (t, (w, perm)) in [(&ws, &perm_s), (&ww, &perm_w)].into_iter().enumerate() {
                ...
                if cyc {
                    xis[t] = xi_from_order(w, n, &r.comps[0].order, true);
                }
            }
            ...
            judge(
                ...
                format!(
                    "stag ok={} ξ={:.2} / Wil ok={} ξ={:.2} / ξ比 {:.3} (窓 [{}, {}])",
                    oks[0], xis[0], oks[1], xis[1], ratio, BAR_H3_XI_RATIO.0, BAR_H3_XI_RATIO.1
                ),
            );
        }
    }

    // ---- [H4] Lorentzian 整合 (B4 × B1 距離) ----
    {
        // リング N=202
        let n = 202;
        ...
        let (v_all, r2) = front_fit(&all_pairs);
        ...
        judge(
            &mut score,
            "B4",
            "HOLD-4 リング (Lorentzian: t* vs 再構成距離)",
        ...
        // 開放鎖: 無巻き付き (path 距離で線形)
        let ok_order = !h1_b1_order.is_empty();
        let (v1, r21, np1) = if ok_order {
            let mut pos1 = vec![0usize; n1];
            for (t, &nd) in h1_b1_order.iter().enumerate() {
                pos1[nd] = t;
            }
        ...
        judge(
            &mut score,
            "B4",
            "HOLD-4 開放鎖 (無巻き付き — 位相を因果が追認)",
            ok_order && r21 >= BAR_H4_R2,
            format!("v = {:.4} R² = {:.5} ({} 点)", v1, r21, np1),
        );
    }

    // ---- 成績表と裁定 ----
    println!("\n---- 候補別成績 (凍結規則: バー外は調整せず棄却) ----");
    ...
    {
        let j = Json::Obj(vec![
            ("version".into(), Json::Str("v28.2".into())),
            (
                "survived".into(),
                    ...
                    judged
                        ...
                        .map(|(name, ok, detail)| {
                            Json::Obj(vec![
                                ...
                                ("bar_ok".into(), Json::Str(if *ok { "内" } else { "外" }.into())),
        ...
        let p = write_artifact("results/v282_bridge_holdout.json", &j.render());
        println!("\n[成果物] {}", p);
    }

    println!(
        "\n[判定] 生存 {:?} / 棄却 {:?} — 棄却候補の再調整は行わない (bridge_candidates.yml の失敗規則)。",
        survived, rejected
    );
    println!("解釈の上限: 生存候補が示すのは「同じ網から時空 (的構造) が出る」toy 機構 (C3) — bridge law 登録簿は空のまま。");
    println!(
        "\n総合判定: {}",
        if n_instr_fail == 0 {
            "[PASS]"
