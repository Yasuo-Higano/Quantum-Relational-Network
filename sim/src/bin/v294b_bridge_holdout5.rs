//! v29.4b HOLD-5 holdout の初開封と本採点 — 定量計量 (第三十期, PROMPT/11)
//!
//! hold-8..15 (8 系) をここで**初めて生成・観測**する。事前凍結の全要素:
//!   - 生成器・資格採点器・バー: FROZEN BRIDGE READOUT v29 (v28.1) /
//!     FROZEN BRIDGE MATRIX v30 (v29.3) / FROZEN QUANT v31 (v29.4a) —
//!     [H0] が 3 節の SHA-256 逐語一致を検査する。
//!   - seed 系列: sha256(SECRET+":"+id) — SECRET は v29.2 でコミットメント公表、
//!     v29.4a で開示済み。train/val seed の一致も [H0] が再検査する。
//!   - 採点: [H1] 資格 (S4×C3×2 質量 — v29.3 凍結バー) / [H2] 定量 (Δ∞(v̂,v_true)・
//!     未使用源 τ 予言・regulator 間 Δ∞ — v29.4a 凍結バー)。
//!
//! **バー外の結果は調整せず受理する** (bridge_candidates.yml の失敗規範)。候補の
//! 成績は [採点] 行で記録し、[FAIL] トークンは器械破れ (凍結節不一致・採点不能)
//! だけに使う (v28.2 の慣例 — suite の FAIL 計数を汚染しない)。
...
/// 1 ジョブ = (instance, regulator)。資格 (2 質量 × S4 × C3) + 定量 (m=0)。
/// v29.4a の run_job と同一の手続き (holdout 系に適用するだけ — 変更禁止)。
struct JobOut {
    id: String,
    rname: &'static str,
    ring: bool,
...
fn main() {
    self_test();
    println!("=== v29.4b HOLD-5 holdout 初開封と本採点 — 定量計量 (第三十期) ===\n");
    ...
    // ---- [H0] 凍結の完全性 (器械検査) ----
    {
        ...
        let me = read_src("sim/src/bin/v294b_bridge_holdout5.rs");
        let s281 = read_src("sim/src/bin/v281_bridge_train.rs");
        let s293 = read_src("sim/src/bin/v293_bridge_matrix.rs");
        let s294a = read_src("sim/src/bin/v294a_bridge_val.rs");
        ...
        const B31: &str = "// ================== FROZEN QUANT v31 (BEGIN)";
        const E31: &str = "// =================== FROZEN QUANT v31 (END)";
        let same = |a: Option<String>, b: Option<String>| -> (bool, String) {
            match (a, b) {
                (Some(x), Some(y)) => (
                    !x.is_empty() && sha256_hex(x.as_bytes()) == sha256_hex(y.as_bytes()),
                    sha256_hex(x.as_bytes()),
                ),
                _ => (false, "?".into()),
            }
        };
        let (ok29, _) = same(cut(&s281, B29, E29), cut(&me, B29, E29));
        let (ok30, sha30) = same(cut(&s293, B30, E30), cut(&me, B30, E30));
        let (ok31, sha31) = same(cut(&s294a, B31, E31), cut(&me, B31, E31));
        check(
            "[H0] 凍結の完全性 — SECRET コミットメント・train seed・凍結 3 節の逐語一致",
            ok_commit && ok_train && ok29 && ok30
                && sha30 == "93e1aa5cc0aed19ff8488fb8aa98f07ffd28ebda10cb61fd99b85f1a6e59fd8c"
                && ok31,
            format!(
                "commitment {} / train {} / v29 {} / v30 {} / v31 {} ({}…)",
                ok_commit,
                ok_train,
                ok29,
                ok30,
                ok31,
                &sha31[..16.min(sha31.len())]
            ),
        );
        check(
            "[H0b] 定量バーが凍結済み (NaN でない)",
            !BAR_DINF_TRUE.is_nan() && !BAR_TAU_PRED.is_nan() && !BAR_DINF_XREG.is_nan(),
            format!(
                "Δ∞true ≤ {} / τ ≤ {} / xreg ≤ {}",
    ...
    // ---- holdout の初開封: hold-8..15 の seed をここで初めて導出・印字 ----
    let hold_ids: [&str; 8] = [
        "hold-8", "hold-9", "hold-10", "hold-11", "hold-12", "hold-13", "hold-14", "hold-15",
    ];
    println!("\n  [開封] hold seed (本走行で初導出):");
    for id in hold_ids {
        println!("        {} = {}", id, hold5_seed(id));
    }

    // ---- 24 ジョブ (8 instance × 3 regulator) — 独立スレッド (決定的回収) ----
    ...
    // ---- [H1] 資格の採点 (凍結バー — 成績は [採点]、器械破れではない) ----
    let mut n_qual_out = 0usize;
    {
        ...
        n_qual_out = fails.len();
        println!(
            "\n  [採点] [H1] holdout 8 系 × 3 regulator × 2 質量 × S(4) × C(3) = 576 セルの資格: バー外 {} 件 (最小 Spearman = {:.4})",
            n_qual_out, min_sp
        );
        ...
        if n_qual_out > 12 {
            println!("        … 他 {} 件 (JSON に全記録)", n_qual_out - 12);
        }
    }

    // ---- [H2] 定量の採点 (凍結バー) ----
    let (mut max_dinf, mut max_tau, mut max_xreg) = (0.0f64, 0.0f64, 0.0f64);
    let mut n_quant_out = 0usize;
    let mut n_quant_cells = 0usize;
    let mut quant_verdicts: Vec<String> = Vec::new();
    {
        println!("\n  -- [H2] 定量採点 (S = {}, C = {}, m = 0) --", QUANT_S, QUANT_C);
        println!("     instance reg    Δ∞(v̂,v_true)  τ予言max|ln| (中央値)   対数  被覆率  裁定");
        for row in &outs {
            for o in row {
                if !o.quant_ok {
                    n_quant_out += 1;
                    n_quant_cells += 1;
                    quant_verdicts.push(format!("{} {} 採点不能", o.id, o.rname));
                    println!("     {} {}: 採点不能 (資格バー外または対なし) → バー外扱い", o.id, o.rname);
                    continue;
                }
                n_quant_cells += 1;
                max_dinf = max_dinf.max(o.dinf_true);
                max_tau = max_tau.max(o.tau_worst);
                let ok = o.dinf_true <= BAR_DINF_TRUE && o.tau_worst <= BAR_TAU_PRED;
                if !ok {
                    n_quant_out += 1;
                }
                quant_verdicts.push(format!(
                    "{} {} {}",
                    o.id,
                    o.rname,
                    if ok { "バー内" } else { "バー外" }
                ...
                for j in (i + 1)..row.len() {
                    if row[i].quant_ok && row[j].quant_ok {
                        ...
                        parts.push(format!(
                            ...
                            row[j].rname,
                            x,
                            if x <= BAR_DINF_XREG { "" } else { "**" }
                        ));
                    }
                }
            }
            println!("     {:7}  {}", row[0].id, parts.join("  "));
        }
        n_quant_out += n_xreg_out;
        println!(
            "\n  [採点] [H2] 定量: セル {} 件中 バー外 {} 件 / max Δ∞(v̂,v_true) = {:.4} (バー {}) / max τ予言 = {:.4} (バー {}) / max xreg = {:.4} (バー {})",
            n_quant_cells, n_quant_out, max_dinf, BAR_DINF_TRUE, max_tau, BAR_TAU_PRED, max_xreg, BAR_DINF_XREG
        );
    }

    // ---- 器械の健全性 (採点可能性 — これは [FAIL] 対象) ----
    check(
        "[H3] 採点可能性 — 全 24 (instance × regulator) で定量セルが採点できた",
        outs.iter().flatten().all(|o| o.quant_ok),
        format!(
            "採点不能 {} 件",
            outs.iter().flatten().filter(|o| !o.quant_ok).count()
        ),
    );

    // ---- 成績表と裁定 ----
    let all_in = n_qual_out == 0 && n_quant_out == 0;
    println!("\n[裁定] HOLD-5 holdout (事前凍結バーでの本採点 — 調整なし・結果をそのまま受理):");
    println!(
        "    資格 (576 セル): バー外 {} 件 / 定量 ({} セル + xreg): バー外 {} 件 → {}",
        n_qual_out,
        n_quant_cells,
        n_quant_out,
        if all_in {
            "全セル バー内 — S×C 合成 readout と定量計量 (v̂・τ 予言・regulator 間一致) が新鮮な holdout で生存"
        } else {
            "バー外あり — 該当セルの棄却/限界を確定表に記録 (調整・再採点はしない)"
        }
    );
    ...
    // ---- 成果物 ----
    {
        ...
        for row in &outs {
            for o in row {
                cells.push(Json::Obj(vec![
                    ...
                    ("ring".into(), Json::Bool(o.ring)),
                    ("min_spearman".into(), Json::Num(o.min_sp)),
                    ("n_qual_fails".into(), Json::Num(o.qual_fails.len() as f64)),
        ...
        let j = Json::Obj(vec![
            ("version".into(), Json::Str("v29.4b".into())),
            ("n_qual_out".into(), Json::Num(n_qual_out as f64)),
            ("n_quant_out".into(), Json::Num(n_quant_out as f64)),
            ("max_dinf_true".into(), Json::Num(max_dinf)),
            ("max_tau_pred".into(), Json::Num(max_tau)),
            ("max_xreg".into(), Json::Num(max_xreg)),
            ...
            ("all_in".into(), Json::Bool(all_in)),
            ("cells".into(), Json::Arr(cells)),
        ]);
        let p = write_artifact("results/v294b_bridge_holdout5.json", &j.render());
        println!("\n[成果物] {}", p);
    }
