...
fn main() {
    ...
    println!("観測窓 = 最初の 16 サイト (S_max = 16·ln2 = {:.2})。宇宙は 16 → 96 サイトへ成長。\n", 16.0 * (2.0f64).ln());
    let va = run(false);
    let vb = run(true);
    ...
    let last = va.len() - 1;
    println!("\n  シナリオ A (真空到着):");
    println!("   - 窓のエントロピー: {:.2} → {:.2} へ成長 = 時間の矢", va[0].1, va[last].1);
    ...
    println!("   - 矢の判定 (戻り ≤ 上昇の 15%): {}", pass(max_dip < 0.15 * rise));
    println!("   - 全系は純粋なまま: S_全系 = {:.1e} {} (矢は大域エントロピーなしで生じる!)",
        va[last].2, pass(va[last].2 < 1e-6));
    println!("  シナリオ B (熱的到着): 窓は最初から飽和域 ({:.2}→{:.2}) — 歴史も矢もない",
        vb[0].1, vb[last].1);
    println!("\n結論: 「過去はなぜ低エントロピーだったか」の答えの候補:");
    println!("      (i) 過去は自由度が少なかった (S ≤ ln dim: 小さい宇宙は低エントロピーしか持てない)");
    println!("      (ii) 膨張で新しく入るモードは断熱定理により真空で生まれる (Bunch–Davies, v2.4)");
