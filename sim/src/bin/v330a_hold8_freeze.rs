//! v33.0-A HOLD-8 の凍結 — factorization × interaction order × observation contract (PROMPT/13 §5)
//!
//! 第三十二期の全器械 (v32.2 OperationalNet / v32.3 recovery / v32.4 応答階層 /
//! v32.5 hypergraph / v32.1 SupportNoiseCertificate) を凍結し、**識別可能性の境界**を
//! 新鮮 holdout で採点する準備をする。HOLD-8 は「正解グラフを何本当てたか」ではなく、
//!
//!   selective risk = 0 (回答セルの誤り 0)
//!   impossibility recall = 1 (非識別セルの正しい棄却/同値類)
//!   answerable recall = 1 (回答可能セルの回答 — coverage 単独は最大化しない)
//!
//! を採点する (v31.0 で型として凍結した採点原則の第二執行 — HOLD-7 の後継)。
//!
//! セル 8 クラス 20 セル:
//!   F1 site net → Exact / F2 mode (DFT) net → Exact (同一閉包・別 orbit) /
//!   F3 number op のみ → Abstain(Insufficient) / F4 非互換 2 net → EquivalenceClassOnly /
//!   F5 中心非自明 → SuperselectionSectors / F6 odd 入力 → ordinary 構成時拒否 +
//!   graded 化 (偶双線形) → パリティ sector
//!   I1 quadratic / I2 t-V (V 不可視 = ŵ = t²) / I3 相関 hopping (条件付き分離) /
//!   I4 pair hopping / I5a 三体混合 (hyperedge 検出) / I5b その null 対 /
//!   I6 H↔−H 対 (密度 = 同値・coherent = 分離) / I7 磁束対 (同上)
//!   M1 局所 U × 置換の変成対 (同一 orbit) / M3 演算子基底の可逆再結合 (不変) /
//!   M4 可換子 margin 以下のノイズ → Abstain / M5 support margin 以下の弱辺 → Abstain
//!
//! 開封順序 (HOLD-5/6/7 と同一):
//!   v33.0-A (本コミット) = 生成器・採点器・バー・棄却規則の凍結 + SECRET
//!   コミットメント公表 + train 採点 (可視シード 33001)
...
//!
//! 実行: cargo run --release --bin v330a_hold8_freeze
...
fn main() {
    uft_sim::self_test();
    println!(
        "=== v33.0-A HOLD-8 の凍結 — factorization × interaction × observation contract (PROMPT/13 §5) ===\n"
    );
    ...
    {
        let mut bad = Vec::new();
        if HOLD8_COMMITMENT.len() != 64 || !HOLD8_COMMITMENT.chars().all(|c| c.is_ascii_hexdigit())
        {
            bad.push("コミットメントが sha256 hex でない".to_string());
        }
        if let Err(e) = operational_net_self_test() {
            bad.push(format!("operational_net_self_test: {}", e));
        }
        // 凍結 recovery の 3 anchor (v32.3 の再現)
        let mut rng = Rng::new(7);
        let site = site_gens();
        let det = recover_factorization(&build_net(&site, 8, 0.0, &mut rng), &site, 8);
        if det.reading
            != (FactorizationReading::ExactUpToLocalUnitaryAndPermutation {
                local_dims: vec![2, 2, 2],
            })
        {
            bad.push("site recovery anchor が崩れた".to_string());
        }
        check(
            "[A0] sha256(SECRET) コミットメント公表 + 凍結器械の自己検査 (net/recovery anchor)",
            bad.is_empty(),
            if bad.is_empty() {
                format!("sha256(SECRET) = {}…", &HOLD8_COMMITMENT[..16])
            } else {
                format!("{:?}", bad)
            },
        );
    ...
    // ---- [A1] 採点器の健全性 (裁定語彙と強制回答の検出) ----
    {
        let mut bad = Vec::new();
        // 強制回答が selective risk に数えられること (人工セル)
        let cell = Cell {
            name: "sanity-forced".into(),
            answerable: false,
            expect: Expect::FactorAbstain(
                FactorizationAbstainReason::InsufficientOperationalGenerators,
            ),
            task: Task::Recover { gens: site_gens(), sigma: 0.0 },
        };
        let v = readout(&cell.task, 1);
        let sc = score_cell(&cell, &v);
        if !(sc.answered && !sc.correct) {
            bad.push("強制回答が検出されない (site net は Exact を返すはず)".to_string());
        }
        check(
            "[A1] 採点器の健全性 — 非識別セルへの強制回答を誤りとして数える (人工セルで確認)",
            bad.is_empty(),
            if bad.is_empty() {
                "強制回答 = FAIL の採点が機能".into()
            } else {
                format!("{:?}", bad)
            },
        );
    ...
    // ---- [A2] train 採点 (可視シード) ----
    {
        let (scores, risk, imp_recall, ans_recall) = run_and_score(HOLD8_TRAIN_SEED);
        println!("      train (シード {}) のセル表:", HOLD8_TRAIN_SEED);
        for s in &scores {
            println!(
                "        {} {:22} [{}] {}",
                if s.correct { "✓" } else { "✗" },
        ...
        let ok = risk == 0.0 && imp_recall == 1.0 && ans_recall == 1.0 && scores.len() == 20;
        check(
            "[A2] train 満票 — selective risk 0・impossibility recall 1・answerable recall 1 (20 セル)",
            ok,
            format!(
                "セル {} / selective risk = {:.3} / impossibility recall = {:.3} / answerable recall = {:.3}",
                scores.len(),
                risk,
                imp_recall,
                ans_recall
    ...
    println!(
        "\n[判定] {}",
        if nfail == 0 {
            "HOLD-8 は凍結された — 生成器・採点器・バー・棄却規則は本コミットで固定。開封 (v33.0-B) は SECRET 開示と同時・調整なし"
        } else {
            "**凍結の破れ** — 器械と採点器を修正せよ"
        }
    );
