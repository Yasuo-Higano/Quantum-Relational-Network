...
fn main() {
    ...
    let big_l = 120.0;
    let dt = 4e-4;
    println!("=== v3.4 Unruh 効果: 加速検出器の応答 (a={}, 数値FT: L={}, ε={}) ===\n", a, big_l, eps);
    ...
    for &om in &[0.5f64, 1.0, 2.0] {
        ...
        println!("  ω={:.1}: F(+ω) = {:+.2e} (→0),  F(-ω) = {:+.4} (自発放出は有限)",
            om, f_pos, f_neg);
    }
    ...
    for &om in &[0.25f64, 0.5, 0.75, 1.0, 1.25] {
        ...
        max_terr = max_terr.max((teff - a / (2.0 * std::f64::consts::PI)).abs() / (a / (2.0 * std::f64::consts::PI)));
        println!("  {:.2}  {:+.3e}  {:.5e}  {:.5e}  {:.5}", om, f_pos, ratio, boltz, teff);
    }
    let t_unruh = a / (2.0 * std::f64::consts::PI);
    println!("  => 有効温度の理論値 T = a/2π = {:.5}, 最大相対誤差 {:.1e}  {}",
        t_unruh, max_terr, pass(max_terr < 0.01));
    ...
    let vdev = vals.iter().map(|v| (v / vmean - 1.0).abs()).fold(0.0f64, f64::max);
