//! v18.5 磁束つき φ_B — 位相干渉は θ23 の壁を破るか
//!
//! v18.4 は「Majorana 方向は正しいがプロファイルの豊かさが不足 (特異 M_R 0 件 =
//! 良条件すぎる)」を確定した。本バイナリは CP アークの型 (位相が構造を作る) を
//! B−L 破れ場に適用する: φ_B = 周期 Gaussian × e^{2πi(wx·x/nx + wy·y/ny)} —
//! 巻き数 (wx, wy) ∈ {0..3}² が新離散データ。位相の干渉が Majorana テクスチャに
//! 非対角/near-singular 構造を作り、シーソーの階層反転 (=大混合) を可能にするかが
//! 問い。B-configs = 位置 3×2 × 幅 {2,4,8} × 巻き 16 = 288 種 (一様)。
//! 事前登録: (a) sin²θ23 帯が 0.55 被覆 = 位相が壁を破る (PMNS 昇格の道) /
//! (b) 上端 1.5 倍改善 = 部分前進 / (c) 変化なし = 巻きも不足 (ν_R 別幾何 /
//! Weinberg 直行へ)。機構指標: 特異 M_R 率 (>0 が干渉の証拠) と
//! P(sin²θ23 ∈ [0.4, 0.7])。プロトコル・ゲートは v18.4 と同一。
...
fn main() {
    self_test();
    println!("=== v18.5 磁束つき φ_B: 位相干渉は θ23 の壁を破るか ===\n");
    println!("設計 (データ盲目): φ_B = Gaussian × e^{{2πi(wx·x/nx + wy·y/ny)}} — 巻き数 (wx,wy) ∈ {{0..3}}² が新離散データ。");
    println!("  M_R[i][j] = M₀·∫ψ_νi ψ_νj φ_B (複素プロファイル)。m_ν = Y_ν M_R⁻¹ Y_νᵀ。特異は skip 計数。");
    println!(
        "事前登録: (a) sin²θ23 帯が 0.55 被覆 = 位相が壁を破る / (b) 上端 1.5 倍改善 = 部分前進 /"
    );
    println!("          (c) 変化なし = 巻きも不足 (ν_R 別幾何/Weinberg へ)\n");
    ...
    // B プロファイル集合 (位置 3×2 × 幅 3 × 巻き 16 = 288)
    ...
    let phibs: std::sync::Arc<Vec<Vec<(f64, f64)>>> = std::sync::Arc::new(phib_list);
    println!(
        "[0] B プロファイル {} 種 (位置 {}×{} × 幅 {:?} × 巻き 4×4)",
        phibs.len(),
        bxs.len(),
        bys.len(),
        sigbs
    ...
    let handles: Vec<_> = g0
        ...
        .map(|(isg, &s0)| {
            ...
            std::thread::spawn(move || {
                ...
                let mut n_sing = 0u64;
                let mut n_eval = 0u64;
                let mut w23mid = 0.0f64;
                let mut w23tot = 0.0f64;
                for sl in 0..6 {
                    for se_ in 0..6 {
                        for ab in 0..nc * nc {
                ...
                (
                    isg, sig_w, h12, h23, h13, hr, jl_pos, jl_tot, n_sing, n_eval, w23mid, w23tot,
                )
            })
        })
        .collect();
    ...
    let mut w23mid = 0.0f64;
    let mut w23tot = 0.0f64;
    for (isg, sw, a, b, c, d, jp, jt, ns, ne, wm, wt) in rs.iter() {
        let _ = isg;
        let w = (sw - smax).exp();
        let mut aa = a.clone();
    ...
    // ---- [3] 帯と採点 ----
    println!("\n[3] PMNS 帯 (磁束つき M_R, 16/50/84%) と v18.4 (無巻き) との比較:");
    let rows = [
        ("sin²θ12", 0.307f64, &h12, [0.003, 0.018, 0.357]),
        ("sin²θ23", 0.55, &h23, [0.003, 0.003, 0.278]),
        ("sin²θ13", 0.022, &h13, [0.004, 0.041, 0.344]),
    ];
    let mut cov23 = false;
    let mut top23 = 0.0f64;
    ...
    println!(
        "    sign(J_lep) > 0 の確率: {:.2} / P(sin²θ23 ∈ [0.4, 0.7]) = {:.4} / 特異 M_R 率 {:.4}%",
        jl_pos / jl_tot.max(1e-300),
        w23mid / w23tot.max(1e-300),
        100.0 * n_sing as f64 / (n_eval + n_sing).max(1) as f64
    );
    println!(
        "\n    => 事前登録: {}",
        if cov23 {
            "(a) θ23 の帯が 0.55 を被覆 — 位相干渉が壁を破った (PMNS 昇格の道が開く)"
        } else if top23 > 0.278 * 1.5 {
            "(b) 上端が 1.5 倍以上改善したが届かない — 部分前進 (巻きの拡張/複合が次)"
        } else {
            "(c) 実質変化なし — 巻き位相も不足 (ν_R 別幾何 / Weinberg 直行へ)"
        }
    );

    // ---- artifact ----
    let j = Json::Obj(vec![
        ("version".into(), Json::Str("v18.5".into())),
        (
            "bands".into(),
            Json::Arr(
                vec![
        ...
        ("p23_mid".into(), Json::Num(w23mid / w23tot.max(1e-300))),
        ("n_eval".into(), Json::Int(n_eval as i64)),
    ]);
    let p = write_artifact("results/v185_fluxphib.json", &j.render());
    println!("\n[artifact] {}", p);
