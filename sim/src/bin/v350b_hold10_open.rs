//! v35.0-B HOLD-10 holdout の開封と本採点 — 第三十四期の期末判定
//!
//! 開封手続き (HOLD-5..9 と同一・調整なし):
//!   [H0] SECRET 開示 — sha256(SECRET) = v350a のコミットメント c3a62f1b… と一致・
//!        FROZEN-HOLD10 区間が v350a と逐語一致・lib pin 8 モジュール不変
//!   [H1] holdout 初生成 — シード = sha256(SECRET + "-seed") の先頭 16 hex (u64)。
//!        HOLD-10S 30 セル + HOLD-10C 640 セルを凍結生成器で生成し凍結採点器で採点
//!   [H2] 凍結バー 16 指標の本採点 (S 4 + C 12) — 一切の調整なし
//!
//! 期末判定の規約 (v34.6 で凍結): 外部 D2-R 報告 / 実データ上の事前登録予測 /
//! 新しい厳密 no-go のいずれも無い場合、synthetic holdout が満票でも期末表現は
//! 「instrumental closure」に留める (「QRN の物理的前進」とは呼ばない)。
...
/// lib pin — v350a の凍結値と同一 (開封時に不変を照合)
...
/// 開示された SECRET (コミットメント照合は [H0] が機械実行)
const SECRET: &str = "HOLD10-51335c5367db077ab63c2b9adce8f9c0";
...
fn main() {
    self_test();
    println!("=== v35.0-B HOLD-10 開封 — holdout 本採点 (調整なし) ===\n");
    ...
    // ---- [H0] 開封手続き ----
    {
        let sha = sha256_hex(SECRET.as_bytes());
        let commit_ok = sha == SECRET_COMMITMENT;
        let src_a =
            fs::read_to_string(format!("{}/sim/src/bin/v350a_hold10_freeze.rs", root)).unwrap_or_default();
        let src_b =
            fs::read_to_string(format!("{}/sim/src/bin/v350b_hold10_open.rs", root)).unwrap_or_default();
        let frozen_of = |s: &str| -> String {
            let b = s.find("// ==== FROZEN-HOLD10-BEGIN ====").unwrap_or(0);
            let e = s
                .find("// ==== FROZEN-HOLD10-END ====")
                .map(|x| x + "// ==== FROZEN-HOLD10-END ====".len())
                .unwrap_or(0);
            s[b..e].to_string()
        };
        let verbatim = !src_a.is_empty() && frozen_of(&src_a) == frozen_of(&src_b);
        let mut pin_ok = true;
        for (name, pin) in LIB_PINS {
            let src = fs::read_to_string(format!("{}/sim/src/{}.rs", root, name)).unwrap_or_default();
            if &sha256_hex(src.as_bytes())[..16] != pin {
                pin_ok = false;
            }
        }
        // v350a 側のコミットメント定数とも一致していること (凍結の同一性)
        let commit_in_a = src_a.contains(SECRET_COMMITMENT);
        check(
            "[H0] SECRET 開示 = コミットメント照合・FROZEN-HOLD10 逐語一致・lib pin 8 本不変",
            commit_ok && verbatim && pin_ok && commit_in_a,
            format!(
                "sha256({}…) = {}… / 逐語一致 {} / pin {}",
                &SECRET[..14.min(SECRET.len())],
                &sha[..16],
                verbatim,
                pin_ok
            ),
    ...
    // ---- [H1] holdout 初生成・本採点 ----
    let seed = {
        let s = sha256_hex(format!("{}-seed", SECRET).as_bytes());
        u64::from_str_radix(&s[..16], 16).unwrap()
    };
    println!("\n[H1] holdout シード = {} (sha256(SECRET + \"-seed\") 先頭 16 hex — SECRET 導出)", seed);
    ...
    {
        let mut check2 = |name: &str, ok: bool, detail: String| {
            println!(
                "  [{}] {}  {}",
                if ok { "PASS" } else { "FAIL" },
                name,
        ...
        check2(
            "[H1] holdout 本採点 — 16 指標",
            all_pass,
            format!(
                "回答 {} (W = {})・被覆 {}/{}・境界棄却 {}/{}・misspec {}/{}",
                rep.n_answers,
                rep.n_wrong,
                rep.n_cover,
                rep.n_intervals,
                rep.boundary_abstained,
                rep.boundary_total,
                rep.misspec_detected,
                rep.misspec_total
            ),
        );

        // ---- [H2] population 上限の報告 (「満票」を超える報告義務) ----
        let upper = if rep.n_wrong == 0 && rep.n_answers > 0 {
            zero_error_upper_bound(rep.n_answers, 0.95)
        } else {
            f64::NAN
        };
        check2(
            "[H2] population-risk の片側 95% 上限の報告 (観測誤りだけで完成としない)",
            rep.n_answers >= BAR_ANSWERS_MIN,
            format!(
                "wrong_promotion 上限 = {:.5} (W = {} / 回答 {}) — セル i.i.d. (凍結分布からの独立抽選) の登録仮定下",
                upper, rep.n_wrong, rep.n_answers
            ),
        );
    }
    ...
    println!(
        "\n[判定] {}",
        if nfail == 0 {
            "HOLD-10 開封 満票 — ただし期末表現は「instrumental closure」に留める:\n       外部 D2-R 報告 0・実データ上の事前登録予測 0・(第四の no-go は本期の\n       計画内成果であり完成条件の「新しい」no-go には数えない)。synthetic\n       holdout の満票は器械の閉包であって QRN の物理的前進ではない (v34.6 凍結)。"
        } else {
            "**開封で不成立あり** — 凍結バーどおり不成立として確定し記録する (調整しない)"
        }
    );
    println!(
        "\n総合判定: {}",
