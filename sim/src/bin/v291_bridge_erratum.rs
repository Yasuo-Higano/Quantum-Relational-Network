//! v29.1 bridge 裁定の Integrity Erratum (第三十期 第0課題, PROMPT/11)
//!
//! 発見 (PROMPT/11, 独立監査): lib の `linfit` は y = a + b·x に対し **(切片 a,
//! 傾き b)** を返すが、v28.2 `xi_from_order` と v28.3 `xi_envelope` は
//! `let (slope, _) = linfit(..)` と書き**切片を減衰率に使用** — HOLD-3 の
//! 「ξ 比」は ξ = −1/切片 であり、裁定 (B2 棄却) を反転させる採点器バグ。
//! バグは凍結節の**外** (採点側) — 読み出し規則・バーは無傷。
//!
//! 本バイナリの三段構造:
//!   [E0] 凍結節 SHA-256 = v281 (読み出し規則の無変更) + TRAIN-1 アンカー
//!   [E1] **バグの再現**: 切片版推定器 (偶距離 = v28.2 / 上包絡 = v28.3) を
//!        そのまま再実装し、公表値 (as-run) を印字精度で再現 — 同一入力・同一
//!        バグであることの機械証明 (これを経ずに修正だけ走らせない)
//!   [E2] **修正裁定**: linfit_typed → KernelDecayLength (傾きの負・有限を型が
//!        強制) で HOLD-3 を再採点。バー [0.8, 1.25] は不変。
//!   [E3] 修正と無関係な全セル (H1×3, H2×5, H4×2) の裁定が v28.3 と一致
//!
//! 意味論の同時修正 (PROMPT/11):
//!   - 実装済み候補の正名: B3 → **B3-COV** (density connected-covariance) /
//!     B4 → **B4-DENSITY-FRONT** (空間核×密度前線の**合成** readout)。
//!     元の B3-QFI / B4-COMMUTATOR は未試験候補として v29.2 で再登録。
//!   - ξ は「物理的相関長」でなく **KernelDecayLength** (核冪・前因子未分離)。
//!   - NodeState はノード因子分解を**入力**している (隠したのは座標・隣接・
//!     ラベルであって分解ではない) — 主張の scope は「与えられた因子分解の下で」。
//!   - 棄却 B1 の未使用チャネル得点は生存候補の根拠に流用しない。
//! outcome preservation 禁止: 結果 (B2 再生存を含む) をそのまま受理する。
//! [FAIL] は器械破れ (E0/E1/E3) のみ。裁定は [採点] 行。

use std::collections::BTreeMap;
use uft_sim::qrn_core::KernelDecayLength;
use uft_sim::*;
...
fn pos_of(order: &[usize], n: usize) -> Vec<usize> {
...
/// as-run 再現 (v28.2): 偶距離 d ∈ {4,6,…,20} の ⟨ln w⟩ を linfit し
/// **切片**を減衰率に使う (バグの逐語再実装 — 再現専用・修正裁定には使わない)
fn xi_buggy_even(w: &[f64], n: usize, order: &[usize], cyclic: bool) -> f64 {
    let pos = pos_of(order, n);
    ...
    let (intercept_misused_as_slope, _) = linfit(&xs, &ys);
    -1.0 / intercept_misused_as_slope
}

/// as-run 再現 (v28.3): 上包絡 d ∈ {3,…,20} を linfit し**切片**を減衰率に使う
fn xi_buggy_env(w: &[f64], n: usize, order: &[usize], cyclic: bool) -> f64 {
    let pos = pos_of(order, n);
    let dist = |i: usize, j: usize| -> usize {
        let d = (pos[i] as isize - pos[j] as isize).unsigned_abs();
        if cyclic {
...
/// 修正版 (v29.1): 上包絡 d ∈ {3,…,20} を linfit_typed し、**傾き**から
/// KernelDecayLength を構成 (負・有限を型が強制 — fail-closed)
fn xi_typed_env(
    w: &[f64],
    n: usize,
    order: &[usize],
    cyclic: bool,
) -> Result<KernelDecayLength, &'static str> {
    let pos = pos_of(order, n);
    let dist = |i: usize, j: usize| -> usize {
        let d = (pos[i] as isize - pos[j] as isize).unsigned_abs();
        if cyclic {
    ...
    let fit = linfit_typed(&xs, &ys)?;
    KernelDecayLength::try_from(fit)
}
...
fn main() {
    self_test();
    println!("=== v29.1 bridge 裁定の Integrity Erratum — 切片/傾きバグの再現と修正裁定 (第三十期 第0課題) ===\n");
    ...
    // ---- [E0] 凍結節 = v281 + TRAIN-1 アンカー ----
    {
        ...
        let s291 = read_src("sim/src/bin/v291_bridge_erratum.rs");
        let sha_ok = match (cut(&s281), cut(&s291)) {
            (Some(a), Some(b)) => sha256_hex(a.as_bytes()) == sha256_hex(b.as_bytes()),
            _ => false,
        };
        check(
            "[E0] 凍結節 SHA-256 = v281 (核・パイプライン・バーの無変更 — バグは採点側)",
            sha_ok,
            format!("一致 {}", sha_ok),
        );
    }

    // ---- HOLD-3 の系 (v282/v283 と同一の構成・シード) ----
    let n = 402;
    let m_phys = 0.05;
    ...
    let truth_ring = |a: usize, b: usize| {
        let d = (a as isize - b as isize).unsigned_abs();
        d.min(n - d) == 1
    };
    let kernels: [(&str, Vec<f64>, Vec<f64>); 3] = [
        ("B1", w_b1_gauss(&shuf_s), w_b1_gauss(&shuf_w)),
        ("B2", w_b2_gauss(&shuf_s), w_b2_gauss(&shuf_w)),
        ("B3", w_b3_gauss(&shuf_s), w_b3_gauss(&shuf_w)),
    ];
    // 再構成 (裁定は全採点で共通 — 位相・隣接は H3 で全核バー内だった)
    let mut orders: Vec<(Vec<usize>, Vec<usize>, bool)> = Vec::new(); // (stag順, wil順, 位相ok)
    for (_, ws, ww) in &kernels {
        let rs = reconstruct(ws, n);
        let rw = reconstruct(ww, n);
        let ok_s = rs.comps.len() == 1
            && rs.comps[0].topology == 0
            && rs.comps[0].order.len() == n
            && adjacency_accuracy(&rs.edges, &perm_s, &truth_ring) >= BAR_ADJACENCY
            && rs.bridges.is_empty();
        let ok_w = rw.comps.len() == 1
            && rw.comps[0].topology == 0
            && rw.comps[0].order.len() == n
            && adjacency_accuracy(&rw.edges, &perm_w, &truth_ring) >= BAR_ADJACENCY
            && rw.bridges.is_empty();
        orders.push((rs.comps[0].order.clone(), rw.comps[0].order.clone(), ok_s && ok_w));
    }

    // ---- [E1] バグの再現 (as-run 公表値との照合) ----
    {
        // v28.2 (偶距離・切片) の公表値: ξ (2 桁) と比 (3 桁)
        let pub_even: [(f64, f64, f64); 3] = [
            (0.19, 0.27, 0.701),
            (0.53, 1.00, 0.528),
            (0.15, 0.19, 0.797),
        ];
        // v28.3 (上包絡・切片) の公表値
        let pub_env: [(f64, f64, f64); 3] = [
            (0.26, 0.29, 0.902),
            (0.81, 1.07, 0.755),
            (0.19, 0.20, 0.968),
        ];
        let mut bad = Vec::new();
        for (k, (name, ws, ww)) in kernels.iter().enumerate() {
            let (os, ow, _) = &orders[k];
            let (es, ew) = (
                xi_buggy_even(ws, n, os, true),
                xi_buggy_even(ww, n, ow, true),
            );
            let (vs, vw) = (xi_buggy_env(ws, n, os, true), xi_buggy_env(ww, n, ow, true));
            let r_even = es / ew;
            let r_env = vs / vw;
            let (pe, pw, pr) = pub_even[k];
            let (qe, qw, qr) = pub_env[k];
            if (es - pe).abs() > 5e-3
                || (ew - pw).abs() > 5e-3
                || (r_even - pr).abs() > 5e-4
            {
                bad.push(format!(
                    "{} 偶距離: 再現 {:.2}/{:.2} 比 {:.3} vs 公表 {:.2}/{:.2} 比 {:.3}",
                    name, es, ew, r_even, pe, pw, pr
                ));
            }
            if (vs - qe).abs() > 5e-3 || (vw - qw).abs() > 5e-3 || (r_env - qr).abs() > 5e-4 {
                bad.push(format!(
                    "{} 上包絡: 再現 {:.2}/{:.2} 比 {:.3} vs 公表 {:.2}/{:.2} 比 {:.3}",
                    name, vs, vw, r_env, qe, qw, qr
                ));
            }
        }
        check(
            "[E1] バグの再現 — 切片版推定器が v28.2/v28.3 の公表値を印字精度で再現 (同一入力・同一バグの機械証明)",
            bad.is_empty(),
            if bad.is_empty() {
                "6 系列 × 2 推定器の全公表値を再現 — 公表 ξ は −1/切片 だったことが確定".into()
            } else {
                format!("{:?}", bad)
            },
        );
    ...
    // ---- [E2] 修正裁定 (傾き → KernelDecayLength, バー不変) ----
    {
        println!(
            "  (導出参考: κ_s = arsinh(ma) = {:.6} / κ_w ≈ ln(1+ma) = {:.6} — 期待比 ~{:.3}。修正後の ξ は「核減衰長」であり物理的相関長と呼ばない)",
            (0.05f64).asinh(),
            (1.05f64).ln(),
            (1.05f64).ln() / (0.05f64).asinh()
        );
        let names = ["B1", "B2", "B3-COV"];
        for (k, (_, ws, ww)) in kernels.iter().enumerate() {
            let (os, ow, topo_ok) = &orders[k];
            let xi_s = xi_typed_env(ws, n, os, true);
            let xi_w = xi_typed_env(ww, n, ow, true);
            let (ok, detail) = match (&xi_s, &xi_w) {
                (Ok(a), Ok(b)) => {
                    let ratio = a.value() / b.value();
                    (
                        *topo_ok && ratio >= BAR_H3_XI_RATIO.0 && ratio <= BAR_H3_XI_RATIO.1,
                        format!(
                            "KernelDecayLength stag {:.4} / Wil {:.4} / 比 {:.4} (窓 [{}, {}] — バー不変)",
                            a.value(),
                            b.value(),
                            ratio,
                            BAR_H3_XI_RATIO.0,
                ...
                _ => (false, "減衰長を構成できない (fail-closed)".into()),
            };
            judge(&mut score, names[k], "HOLD-3 二離散化 (修正裁定)", ok, detail);
        }
    }

    // ---- [E3] 修正と無関係な全セルの裁定一致 (v28.3 = as-run) ----
    {
        ...
        // H4 リング + 開放鎖 (供給 = B2, 修正条項 2)
        {
            let nr = 202;
            let ring = RingChain { n: nr };
            let g = ring.init();
            let st = NodeState {
                nodes: nr,
                m: 1,
            ...
            let perm = make_perm(nr, 20260729);
            let shuf = shuffle_state(&st, &perm);
            let w = w_b1_gauss(&shuf);
            let r = reconstruct(&w, nr);
            let cyc = r.comps.len() == 1 && r.comps[0].topology == 0;
            let pos = if cyc {
                pos_of(&r.comps[0].order, nr)
            } else {
                vec![0; nr]
            };
            let mut h = vec![0.0; nr * nr];
            for x in 0..nr {
                let y = (x + 1) % nr;
                h[x + y * nr] = -1.0;
                h[y + x * nr] = -1.0;
            }
            let mut hp = vec![0.0; nr * nr];
            for i in 0..nr {
                for j in 0..nr {
                    hp[i + j * nr] = h[perm[i] + perm[j] * nr];
                }
            }
            let sources: Vec<usize> = (0..8).map(|k| k * nr / 8).collect();
            let base: Vec<f64> = (0..nr).map(|j| shuf.cre[j + j * nr]).collect();
            ...
            // 開放鎖 (供給 B2)
            let ok_open = if let Some(order) = h1_orders.get("B2") {
                let pos1 = pos_of(order, n1);
                ...
                let (_v1, r21) = front_fit(&pairs1);
                r21 >= BAR_H4_R2
            } else {
                false
            };
            run2.push(("B4-H4OPEN", ok_open));
        }
        // v28.3 確定表との一致
        let expected: [(&str, bool); 11] = [
            ("B1-H1", false),
            ("B2-H1", true),
            ("B3-H1", true),
            ("B1-H2", true),
            ...
            ("B4-H4RING", true),
            ("B4-H4OPEN", true),
            ("B1-H1", false), // 重複アンカー (先頭セルの再確認)
        ];
        let mut bad = Vec::new();
        for (name, want) in expected {
            match run2.iter().find(|(nm, _)| *nm == name) {
                Some((_, got)) if *got == want => {}
                Some((_, got)) => bad.push(format!("{}: v28.3={} 再計算={}", name, want, got)),
                None => bad.push(format!("{}: 再計算なし", name)),
            }
        }
        check(
            "[E3] 修正と無関係な 10 セルの裁定が v28.3 (as-run) と一致",
            bad.is_empty(),
            if bad.is_empty() {
                "erratum は HOLD-3 の ξ にのみ波及".into()
            } else {
                format!("{:?}", bad)
            },
        );
        // 成績表へ転記 (裁定は v28.3 のまま — 正名で記録)
        let get = |nm: &str| run2.iter().find(|(x, _)| *x == nm).unwrap().1;
        judge(&mut score, "B1", "HOLD-1 開放端鎖", get("B1-H1"), "橋誤検出 (境界増強) — 候補固有・棄却維持".into());
        judge(&mut score, "B2", "HOLD-1 開放端鎖", get("B2-H1"), "".into());
        judge(&mut score, "B3-COV", "HOLD-1 開放端鎖", get("B3-H1"), "".into());
        judge(&mut score, "B1", "HOLD-2 Z2 (相互作用)", get("B1-H2"), "".into());
        judge(&mut score, "B2", "HOLD-2 Z2 (相互作用)", get("B2-H2"), "".into());
        judge(&mut score, "B3-COV", "HOLD-2 Z2 (相互作用)", get("B3-H2"), "".into());
        judge(&mut score, "B1", "HOLD-2 未使用チャネル", get("UNUSED-H2"), "B1 の得点 — 棄却候補のため生存者の根拠に流用しない".into());
        judge(&mut score, "B4-DENSITY-FRONT", "HOLD-2 因果 (相互作用系)", get("B4-H2"), "距離 = B1 再構成 (合成 readout)".into());
        judge(&mut score, "B4-DENSITY-FRONT", "HOLD-4 リング (Lorentzian)", get("B4-H4RING"), "距離 = B1 再構成 (合成 readout)".into());
        judge(&mut score, "B4-DENSITY-FRONT", "HOLD-4 開放鎖", get("B4-H4OPEN"), "距離 = B2 再構成 (合成 readout, 修正条項 2)".into());
    }

    // ---- 訂正後の確定成績表 ----
    println!("\n---- 訂正後の確定成績 (正名: B3-COV = density 共分散核 / B4-DENSITY-FRONT = 空間核×密度前線の合成) ----");
    ...
    {
        let j = Json::Obj(vec![
            ("version".into(), Json::Str("v29.1".into())),
            (
                "survived".into(),
            ...
            (
                "note".into(),
                Json::Str(
                    "v28.2/28.3 の HOLD-3 は切片を減衰率に使用 (裁定無効) — 本表が確定。v28.3 は post-holdout instrument-repair retest と再分類".into(),
                ),
            ),
        ]);
        let p = write_artifact("results/v291_bridge_erratum.json", &j.render());
        println!("\n[成果物] {}", p);
    }

    println!(
        "\n[判定] 訂正後の確定: 生存 {:?} / 棄却 {:?}。B2 の「regulator 物性ゆえ棄却」と B3 の「導出期待に 0.8% 一致」は撤回 (erratum)。生存は「与えられたノード因子分解の下で、ラベル・座標・隣接を入力せず、隠れた 1 次元隣接・位相・因果順を回復した」toy 機構 (C3) — bridge law 登録簿は空のまま。",
        survived, rejected
    );
    println!(
        "\n総合判定: {}",
