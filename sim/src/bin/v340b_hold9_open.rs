//! v34.0-B HOLD-9 の開封 — SECRET 開示・holdout 初生成・本採点 (調整なし)
//!
//! SECRET を開示して holdout を初生成・本採点する。**調整なし**。
//!   [H0] SECRET がコミットメント ef6a8cd9… と一致 + FROZEN-HOLD9 区間が
//!        v340a と逐語一致 (SHA-256) + 器械 lib pin (6 モジュール) 不変
//!   [H1] holdout 20 セル (SECRET 導出シード) の本採点 — セル表と 3 + 5 計量を記録
//!   [H2] 採点の自己整合 — 計量がセル表からの再計算と一致・回答可能 8 / 非識別 12
//!        の区画不変・強制回答が risk に数えられている
//!
//! 実行: cargo run --release --bin v340b_hold9_open
...
/// SECRET (v34.0-B で開示 — sha256 がコミットメントと一致することを [H0] が照合)
pub const HOLD9_SECRET: &str = "HOLD9-44eed4c2f12b4019ed013ec44607b6b5";

/// FROZEN-HOLD9 区間の抽出 (BEGIN..END)
fn frozen_region(src: &str) -> Option<&str> {
    let b = src.find("FROZEN-HOLD9-BEGIN")?;
    let e = src.find("FROZEN-HOLD9-END")?;
    if b < e {
        Some(&src[b..e])
    } else {
        None
...
fn main() {
    ...
    // ---- [H0] SECRET・凍結区間・lib pin ----
    {
        let mut bad = Vec::new();
        let digest = sha256_hex(HOLD9_SECRET.as_bytes());
        if digest != HOLD9_COMMITMENT {
            bad.push(format!("sha256(SECRET) = {} ≠ コミットメント", digest));
        }
        let src_a = std::fs::read_to_string(format!("{}/sim/src/bin/v340a_hold9_freeze.rs", root))
            .unwrap_or_default();
        let src_b = std::fs::read_to_string(format!("{}/sim/src/bin/v340b_hold9_open.rs", root))
            .unwrap_or_default();
        let mut kernel_sha = String::from("????????");
        match (frozen_region(&src_a), frozen_region(&src_b)) {
            (Some(ra), Some(rb)) if ra == rb => {
                kernel_sha = sha256_hex(ra.as_bytes())[..8].to_string();
            }
            _ => bad.push("FROZEN-HOLD9 区間が v340a と一致しない".to_string()),
        }
        ...
        check(
            "[H0] SECRET 開示 = コミットメント一致 / FROZEN-HOLD9 区間の逐語一致 (SHA-256) / lib pin 6 本不変",
            bad.is_empty(),
            if bad.is_empty() {
                format!("SECRET = {} / kernel sha = {}…", HOLD9_SECRET, kernel_sha)
            } else {
                format!("{:?}", bad)
            },
        );
    ...
    let seed = u64::from_str_radix(&sha256_hex(HOLD9_SECRET.as_bytes())[..16], 16).unwrap();
    println!(
        "      holdout シード = {} (SECRET から導出) — 初生成・本採点:",
        seed
    );
    let (cells, counters) = run_cells(seed);
    let (risk, imp_recall, ans_recall) = print_scoreboard(&cells, &counters);
    {
        check(
            "[H1] holdout 20 セルの本採点を執行 (凍結採点器・凍結バー・調整なし)",
            cells.len() == 20,
            format!(
    ...
    {
        let n_ans = cells.iter().filter(|c| c.answer_cell).count();
        let n_imp = cells.len() - n_ans;
        let (r2, i2, a2, forced) = score(&cells);
        // 区画 9/11 は凍結カーネルの answer_cell 宣言から機械的に定まる値
        // (v34.0-B 開封走行の初版はここを 8/12 と誤記し [H2] が正しく FAIL した —
        //  凍結区間・採点・裁定に触れない scaffolding 誤記として訂正・記録)
        let ok = (r2 - risk).abs() < 1e-12
            && (i2 - imp_recall).abs() < 1e-12
            && (a2 - ans_recall).abs() < 1e-12
            && n_ans == 9
            && n_imp == 11
            && counters.admitted_ops == counters.admitted_with_origin
            && counters.exact_readings == counters.exact_with_witness;
        check(
            "[H2] 自己整合 — 計量の再計算一致・回答可能 9 / 非識別 11 の区画不変・出自/証人被覆の分母整合",
            ok,
            format!(
                "risk 再計算 {:.3} = {:.3} / 区画 {}/{} / 強制回答 {} / 出自 {}/{} / 証人 {}/{}",
                r2,
                risk,
                n_ans,
                n_imp,
    ...
    println!(
        ...
        if nfail == 0 {
            if perfect {
                "HOLD-9 開封: 20/20 満票 — 出自・文脈・資源・graded・scale の全軸で、読める境界では読み、読めない境界では棄却した (調整なし)"
            } else {
                "HOLD-9 開封: 満票ならず — 不成立セルは調整せず記録し、機構を次期の設計入力にする (v29.4b/K3-holes と同格)"
            }
        } else {
            "**開封手続きの破れ** — コミットメント・凍結区間・lib pin を確認せよ"
        }
    );
